...
- (void)configureUserInterface
{
    ...
    if (_useMenuLikeSegmentedControl) {
        ...
        actualFrame = CGRectMake(0.0, 0.0, self.view.frame.size.width / (CGFloat)_controllerArray.count, _menuHeight);
    } else if (_menuItemWidthBasedOnTitleTextWidth) {
    ...
    if(!_useMenuLikeSegmentedControl) {
        ...
        _selectionIndicatorView = [[UIView alloc] initWithFrame:actualFrame];
        _selectionIndicatorView.backgroundColor = [UIColor colorWithRed:21.0f/255.0f green:141.0f/255.0f blue:142.0f/255.0f alpha:0.1f];
        UIView *dummyView=[[UIView alloc] initWithFrame:selectionIndicatorFrame];
        dummyView.backgroundColor = _selectionIndicatorColor;
        
        [ _selectionIndicatorView addSubview:dummyView];
    }
